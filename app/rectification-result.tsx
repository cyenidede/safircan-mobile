...
export default function RectificationResultScreen() {
  ...
  if (!result && !error) return <Screen><ActivityIndicator color={colors.sapphire} size="large" /><Text style={[styles.loading,{color:palette.muted}]}>{t('Sonucun hazırlanıyor…')}</Text></Screen>;
  return <Screen>{result ? <RectificationResultCard result={{ birthTime: result.estimatedBirthTime, ascendantSign: result.ascendantSign }} saving={saving} onSave={() => void save()} /> : null}{error ? <Text accessibilityRole="alert" style={[styles.error,{color:palette.danger,backgroundColor:palette.surface}]}>{t(error)}</Text> : null}</Screen>;
}
