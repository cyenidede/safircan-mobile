...
export function DevPremiumPreviewContent() { const [view, setView] = useState<'natal' | 'forecast'>('natal'); const {messages}=useLocale(); const palette=usePalette(); return <Screen>{view === 'natal' ? <PremiumNatalExperience data={DEV_PREMIUM_NATAL} onOpenForecast={() => setView('forecast')} preview /> : <><Pressable accessibilityRole="button" onPress={() => setView('natal')} style={styles.back}><Text style={[styles.backText,{color:palette.sapphire}]}>{messages.premiumPreview.backToChart}</Text></Pressable><AnnualForecastExperience forecast={DEV_ANNUAL_FORECAST} preview /></>}</Screen>; }
