...
test('only-failed questions render the localized normal empty state and detail still supports failed', () => { assert.match(list, /!visibleItems\.length/); assert.match(list, /q\.emptyTitle/); assert.match(detail, /item\.status==='failed'/); });
