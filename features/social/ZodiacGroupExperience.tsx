...
export function ZodiacGroupExperience() {
  const {locale,messages:catalog}=useLocale(); const palette=usePalette(); const m=catalog.zodiacGroup;
  ...
  const load = useCallback(async () => { if (!session?.access_token) { setLoading(false); return; } try { const birthProfile = await getAuthenticatedBirthProfile(session.access_token); if (!birthProfile?.birth_date) { setRestricted('birth_required'); return; } const result = await getZodiacGroup(session.access_token); setRestricted(null); setGroup(result.group); setMessages(result.messages); setUsage(result.usage); } catch (error) { if (error instanceof SocialApiError && error.code === 'age_restricted') setRestricted('underage'); else if (error instanceof SocialApiError && (error.code === 'profile_required' || error.code === 'handle_required')) setRestricted('profile_required'); else setFeedback(m.loadError); } finally { setLoading(false); } }, [m.loadError,session?.access_token]);
  useEffect(() => { void load(); }, [load]);
  const send = async (body: string) => { if (!session?.access_token) return false; setSending(true); setFeedback(''); try { const result = await sendZodiacGroupMessage(session.access_token, body); setUsage(result.usage); await load(); return true; } catch (error) { if (error instanceof SocialApiError && error.usage) setUsage(error.usage); setFeedback(socialErrorMessage(error, m.sendError)); return false; } finally { setSending(false); } };

  return <SafeAreaView edges={['bottom']} style={[styles.safe,{backgroundColor:palette.background}]}><KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} keyboardVerticalOffset={92} style={[styles.safe,{backgroundColor:palette.background}]}>
    <View style={[styles.header,{borderBottomColor:palette.border}]}><Text style={[styles.title,{color:palette.navy}]}>{group ? `${localizeZodiacSignForLocale(group.sign,locale)} ${locale==='tr'?'Grubu':'Group'}` : m.title}</Text><Text style={[styles.subtitle,{color:palette.muted}]}>{group ? m.signedDescription.replace('{sign}',localizeZodiacSignForLocale(group.sign,locale)) : m.description}</Text></View>
    {loading ? <ActivityIndicator color={colors.sapphire} size="large" style={styles.loading} /> : restricted === 'underage' ? <SocialAccessState kind="underage" onPress={() => router.back()} /> : restricted === 'birth_required' ? <SocialAccessState kind="birth_required" onPress={() => router.push('/chart' as Href)} /> : restricted === 'profile_required' ? <View style={styles.restricted}><Text style={[styles.restrictedTitle,{color:palette.navy}]}>{m.profileRequired}</Text><Pressable onPress={() => router.push('/community' as Href)} style={styles.restrictedButton}><Text style={styles.restrictedButtonText}>{m.choosePreferences}</Text></Pressable></View> : <FlatList contentContainerStyle={styles.list} data={messages} keyExtractor={(item) => item.id} renderItem={({ item }) => { const mine = item.sender.userId === user?.id; return <View style={[styles.message,{backgroundColor:palette.surface,borderColor:palette.border}, mine && styles.mine]}><Pressable disabled={mine} onPress={() => router.push({ pathname: '/social-profile', params: { userId: item.sender.userId } } as unknown as Href)} style={styles.messageHeader}><Text style={[styles.sender,{color:palette.navy}, mine && styles.mineText]}>{item.sender.handle ? `@${item.sender.handle}` : mine ? m.you : m.user}</Text>{item.sender.sunSign ? <Text style={[styles.sign, mine && styles.mineMuted]}>{localizeZodiacSignForLocale(item.sender.sunSign,locale)}</Text> : null}</Pressable><Text style={[styles.body,{color:palette.navy}, mine && styles.mineText]}>{item.body}</Text><Text style={[styles.time,{color:palette.muted}, mine && styles.mineMuted]}>{formatDate(item.createdAt,locale,{ day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })}</Text></View>; }} />}
