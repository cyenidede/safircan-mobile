...
export function PrivateChatExperience() {
  const {messages:catalog}=useLocale(); const palette=usePalette(); const m=catalog.socialUi;
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      else setFeedback(m.loadChatError);
  ...
  const deliver = useCallback(async (localId: string, body: string, startedAt: number) => {
    ...
    try {
      ...
      if (!(error instanceof SocialApiError && error.code === 'quota_exceeded')) setFeedback(socialErrorMessage(error, m.sendError));
    } finally {
  ...
  const moderate = async (action: 'block' | 'report') => { if (!session?.access_token || !params.targetUserId) return; await moderateUser(session.access_token, action === 'block' ? { action, targetUserId: params.targetUserId } : { action, reportedUserId: params.targetUserId, reason: 'chat_review' }).then(() => setFeedback(action === 'block' ? m.blocked : m.reported)).catch(() => setFeedback(m.actionError)); };
