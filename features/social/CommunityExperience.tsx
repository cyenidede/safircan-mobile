...
export function CommunityExperience() {
  const {locale,messages}=useLocale(); const palette=usePalette(); const m=messages.community;
  const statusOptions: Array<[RelationshipStatus,string]>=[['single',m.single],['married',m.married],['prefer_not_to_say',m.private]];
  const intentOptions: Array<[RelationshipIntent,string]>=[['meet',m.meet],['serious',m.serious],['friendship',m.friendship]];
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        if (finalState === 'error') setError(socialErrorMessage(cause, messages.common.error));
      }
    } catch (cause) {
      finishAccess('error');
      setError(socialErrorMessage(cause, messages.common.error));
  ...
  const save = async () => {
    ...
    if (!validHandle(normalized)) { setError(messages.common.error); return; }
    if (!relationshipStatus || !relationshipIntent) { setError(m.preferencesCopy); return; }
  ...
  const choosePhoto = async () => {
    ...
    if (!permission.granted) { setError(messages.common.error); return; }
    ...
    if (asset.fileSize && asset.fileSize > 5 * 1024 * 1024) { setError(messages.common.error); return; }
    const mime = asset.mimeType ?? 'image/jpeg';
    if (!['image/jpeg','image/png','image/webp'].includes(mime)) { setError(messages.common.error); return; }
    setSaving(true);
  ...
  if (authLoading || loading) return <Screen><View style={styles.loadingState}><ActivityIndicator color={colors.sapphire} size="large" />{accessState === 'social_provisioning' ? <Text style={[styles.stateCopy,{color:palette.muted}]}>{m.preparing}</Text> : null}</View></Screen>;
  if (!session) return <Screen><View style={styles.intro}><Text style={styles.eyebrow}>SAFIR CAN</Text><Text style={[styles.description,{color:palette.muted}]}>{m.authCopy}</Text></View><Pressable style={styles.primary} onPress={() => router.push('/sign-in')}><Text style={styles.primaryText}>{m.signIn}</Text></Pressable></Screen>;
  ...
  if (accessState === 'error') return <Screen><View style={[styles.stateCard,{backgroundColor:palette.surface,borderColor:palette.border}]}><Text style={[styles.stateTitle,{color:palette.navy}]}>{messages.common.error}</Text><Text style={[styles.stateCopy,{color:palette.muted}]}>{error}</Text><Pressable style={styles.primary} onPress={() => void load()}><Text style={styles.primaryText}>{messages.common.retry.toUpperCase()}</Text></Pressable></View></Screen>;
  const discoveryReady = Boolean(profile?.handle) && !preferencesRequired && Boolean(zodiacGroup);

  return <Screen><View style={styles.intro}><Text style={styles.eyebrow}>SAFIR CAN</Text><Text style={[styles.description,{color:palette.muted}]}>{m.intro}</Text></View>
    {profile ? <View style={[styles.identity,{backgroundColor:palette.surface,borderColor:palette.border}]}><View style={styles.identityHeading}><Pressable onPress={() => void choosePhoto()}>{profile.photoUrl ? <Image source={{ uri: profile.photoUrl }} style={styles.photo} /> : <View style={styles.photoPlaceholder}><Text style={styles.avatarLetter}>S</Text></View>}</Pressable><View style={styles.identityCopy}><Text style={styles.identityLabel}>{m.profile}</Text><Text style={[styles.handle,{color:palette.navy}]}>@{profile.handle}</Text></View>{!preferencesRequired && !editing ? <Pressable onPress={() => setEditing(true)} style={styles.editIcon}><Ionicons color={colors.sapphire} name="pencil-outline" size={20} /></Pressable> : null}</View>
      {editing ? <View style={styles.editForm}><View style={[styles.handleInputRow,{backgroundColor:palette.background,borderColor:palette.border}]}><Text style={styles.at}>@</Text><TextInput autoCapitalize="none" value={handle} onChangeText={(value) => setHandle(value.toLowerCase().replace(/[^a-z0-9_]/g,''))} style={[styles.handleInput,{color:palette.navy}]} maxLength={20} /></View><TextInput value={bio} onChangeText={setBio} placeholder={m.bio} placeholderTextColor={palette.muted} style={[styles.input,styles.multiline,{backgroundColor:palette.background,borderColor:palette.border,color:palette.navy}]} multiline maxLength={300} /><View style={styles.switchRow}><Text style={[styles.switchText,{color:palette.navy}]}>{m.showSign}</Text><Switch value={showZodiacSign} onValueChange={setShowZodiacSign} /></View><View style={styles.editActions}><Pressable onPress={() => setEditing(false)} style={[styles.secondary,styles.editActionButton]}><Text style={styles.secondaryText}>{m.cancel}</Text></Pressable><Pressable disabled={saving} onPress={() => void save()} style={[styles.primary,styles.editActionButton]}>{saving ? <ActivityIndicator color={colors.white} /> : <Text style={styles.primaryText}>{m.save}</Text>}</Pressable></View></View> : null}
    </View> : null}
    {profile && !preferencesRequired ? <SocialNotificationsPanel notifications={notifications} /> : null}
    {preferencesRequired ? <View style={[styles.preferences,{backgroundColor:palette.surface,borderColor:palette.border}]}><Text style={[styles.preferencesTitle,{color:palette.navy}]}>{m.preferences}</Text><Text style={[styles.copy,{color:palette.muted}]}>{m.preferencesCopy}</Text><OptionRow label={m.status} options={statusOptions} value={relationshipStatus} onChange={setRelationshipStatus} /><OptionRow label={m.intent} options={intentOptions} value={relationshipIntent} onChange={setRelationshipIntent} /><Pressable disabled={saving} onPress={() => void save()} style={styles.primary}>{saving ? <ActivityIndicator color={colors.white} /> : <Text style={styles.primaryText}>{m.continue}</Text>}</Pressable></View> : null}
    {discoveryReady && usage ? <View style={[styles.usage,{backgroundColor:palette.sapphireSoft}]}><Text style={styles.usageTitle}>{usage.subscribed ? m.unlimited : `${usage.freeMessagesRemaining} ${m.remaining}`}</Text></View> : null}
    {discoveryReady ? <SocialCard title={m.soulmate} text={m.soulmateCopy} onPress={() => router.push('/soulmate' as Href)} /> : null}
    {discoveryReady && zodiacGroup ? <SocialCard eyebrow={m.group} title={`${localizeZodiacSignForLocale(zodiacGroup.sign,locale)} ${locale==='tr'?'Grubu':'Group'} ${zodiacSymbols[zodiacGroup.sign] ?? ''}`.trim()} text={messages.zodiacGroup.signedDescription.replace('{sign}',localizeZodiacSignForLocale(zodiacGroup.sign,locale))} cta={m.groupCta} onPress={() => router.push('/zodiac-group' as Href)} /> : null}
    {error ? <Text style={styles.error}>{error}</Text> : null}
  </Screen>;
}

function OptionRow<T extends string>({ label, options, value, onChange }: { label: string; options: Array<[T,string]>; value: T | null; onChange: (value:T)=>void }) { const palette=usePalette(); return <View style={styles.optionBlock}><Text style={[styles.optionLabel,{color:palette.navy}]}>{label}</Text><View style={styles.optionWrap}>{options.map(([id,text]) => <Pressable key={id} onPress={() => onChange(id)} style={[styles.option,{borderColor:palette.border}, value === id && styles.optionActive]}><Text style={[styles.optionText,{color:palette.navy}, value === id && styles.optionTextActive]}>{text}</Text></Pressable>)}</View></View>; }
function SocialCard({ eyebrow, title, text, cta, onPress }: { eyebrow?:string;title:string;text:string;cta?:string;onPress:()=>void }) { const palette=usePalette(); return <Pressable onPress={onPress} style={[styles.card,{backgroundColor:palette.surface,borderColor:palette.border}]}><View style={styles.cardCopy}>{eyebrow ? <Text style={styles.cardEyebrow}>{eyebrow}</Text> : null}<Text style={[styles.cardTitle,{color:palette.navy}]}>{title}</Text><Text style={[styles.copy,{color:palette.muted}]}>{text}</Text>{cta ? <Text style={styles.cardCta}>{cta}</Text> : null}</View><Text style={styles.arrow}>›</Text></Pressable>; }
